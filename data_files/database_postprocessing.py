...
file = ['PowerSector.sqlite']  # List of names of databases to be postprocessed
...
for i_file in range(0, len(file)):
    ...
    if result_set["Output_VMat_Cons"]:
        ...
        if not tech and not material_comm:
            ...
            scenario = list(Output_VMat_Cons.scenario)
            scenario = list(dict.fromkeys(scenario))  # To remove duplicates
                    ...
                    for i_regions in range(0, len(regions)):
                        for i_tech in range(0, len(tech)):
                            for i_material_comm in range(0, len(material_comm)):
                                Output_VMat_Cons_dict['file'] = file[i_file]
                                Output_VMat_Cons_dict['scenario'] = scenario[i_scenario]
                                ...
                                check_zeros = True
                                for i_periods in range(0, len(periods)):
                                    VMat_Cons_period = Output_VMat_Cons[(Output_VMat_Cons['scenario'] == scenario[i_scenario]) &
...
if print_set:
    ...
    if result_set["Output_VMat_Cons"]:
        print("\nOutput_VMat_Cons\n\n", Output_VMat_Cons_MERGE.to_string(index=False, float_format='%.2f'))
    if result_set["Output_Emissions"]:
...
if toexcel_set:
    ...
    if result_set["Output_VMat_Cons"]:
        Output_VMat_Cons_MERGE.to_excel(writer, sheet_name='Output_VMat_Cons', index=False)
    if result_set["Output_Emissions"]:
